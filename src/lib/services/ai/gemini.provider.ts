...
import { resolveTimeOverlaps, validateDay, augmentMissingMeals, toHHMM, toMinutes } from './itineraryValidation';
import type { DayIssue } from './itineraryValidation';
import { generateId } from '@/utils/id';
import { getDayCount } from '@/utils/date';

// ---------------------------------------------------------------------------
// 멀티 키 로테이션 — 429 rate limit 시 다음 키로 자동 전환
// ---------------------------------------------------------------------------
// .env.local 예시:
//   GEMINI_API_KEY=AIzaSyB15x...     (메인 키)
//   GEMINI_API_KEYS=AIzaSyB15x...,AIzaSyC22y...,AIzaSyD33z...  (콤마 구분, 선택)
//
// GEMINI_API_KEYS가 있으면 로테이션, 없으면 GEMINI_API_KEY 단일 사용

function getApiKeys(): string[] {
  // GEMINI_API_KEYS (복수) 또는 GEMINI_API_KEY (단수) 둘 다 지원
  // 콤마가 있으면 자동으로 멀티 키로 인식
...
/** 429 발생 시 다음 키로 전환. 전부 소진이면 false 반환 */
...
// ---------------------------------------------------------------------------
// 장소명 중복 제거 유틸 (claude.provider.ts와 동일)
// ---------------------------------------------------------------------------
...
async function callGemini(
  ...
  try {
    ...
    if ((errMsg.includes('429') || errMsg.includes('RESOURCE_EXHAUSTED')) && _retryCount < getApiKeys().length) {
      const rotated = rotateKey();
      if (rotated) {
        console.warn(`[Gemini] 429 → 키 로테이션 후 재시도 (${_retryCount + 1}/${getApiKeys().length})`);
...
// ---------------------------------------------------------------------------
function toTripItems(rawItems: AIGeneratedItem[], stamina: string) {
  ...
  return items.map((item: AIGeneratedItem) => ({
    ...
    ...item,
    // v2: 필드 클램프 — AI가 비정상 값 반환해도 안전하게 보정
