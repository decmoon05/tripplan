...
// ---------------------------------------------------------------------------
// v3 Phase 6.7: 코드 품질 검증 (LLM 호출 불필요한 기계적 체크)
// ---------------------------------------------------------------------------

/** 알려진 체인/프랜차이즈 (대소문자 무시, 부분 매칭) */
const KNOWN_CHAINS = [
  // 일본
  '이치란', '一蘭', 'ichiran', '요시노야', '吉野家', 'yoshinoya',
  '스키야', 'すき家', 'sukiya', '마츠야', '松屋', 'matsuya',
  '코코이치', 'coco壱', 'cocoichi', '사이제리야', 'saizeriya',
  '텐야', '天丼てんや', 'tenya', '오오토야', '大戸屋', 'ootoya',
  '이키나리', 'いきなりステーキ', 'ikinari',
  // 글로벌
  '스타벅스', 'starbucks', '맥도날드', "mcdonald", '버거킹', 'burger king',
  '서브웨이', 'subway', 'kfc', '도미노', 'domino',
  // 한국
  '빽다방', '이디야', 'ediya', '투썸', 'twosome', '할리스', 'hollys',
  '본죽', '김밥천국', '맘스터치',
  // 동남아
  'jollibee', '졸리비',
];

/** 알려진 디저트/간식/빵집 (식사 slot에 부적절) */
const KNOWN_NON_MEAL = [
  // 디저트/빵
  '빵집', '베이커리', 'bakery', '디저트', 'dessert', '파티세리', 'patisserie',
  '롤케이크', '타르트', 'tart', '마카롱', 'macaron',
  '아이스크림', 'ice cream', 'gelato', '젤라토',
  '도넛', 'donut', 'krispy', '크리스피',
  '떡', '과자', '센베이', '煎餅',
  '와플', 'waffle', '크레페', 'crepe',
  // 커피 전문점 (식사 안 되는)
  '커피만', 'coffee only',
...
export interface QualityIssue {
  type: 'chain_restaurant' | 'walk_too_long' | 'non_meal_in_slot';
  item: string;
  detail: string;
  dayNumber: number;
  orderIndex: number;
...
/**
 * 코드로 검증 가능한 품질 이슈 3가지 감지.
 * - 체인 레스토랑 감지 (local-only 시)
 * - 도보 15분 초과 감지
 * - 디저트/간식이 식사 slot에 배치된 경우 감지
 */
export function validateItineraryQuality(
  items: TripItem[],
  restaurantPref: string,
): QualityIssue[] {
  const issues: QualityIssue[] = [];
  const nameLower = (s: string) => s.toLowerCase().replace(/\s+/g, '');

  for (const item of items) {
    const name = nameLower(item.placeNameSnapshot);

    // 1. 체인 감지 (local-only일 때만 — no-preference면 2개까지 허용)
    if (item.category === 'restaurant' || item.category === 'cafe') {
      const isChain = KNOWN_CHAINS.some(c => name.includes(nameLower(c)));
      if (isChain) {
        if (restaurantPref === 'local-only') {
          issues.push({
            type: 'chain_restaurant',
            item: item.placeNameSnapshot,
            detail: `체인/프랜차이즈 감지 — local-only 요청에 부적절`,
    ...
    // 2. 도보 15분 초과
    if (item.transitMode === 'walk' && item.transitDurationMin && item.transitDurationMin > 15) {
      issues.push({
        type: 'walk_too_long',
        item: item.placeNameSnapshot,
        detail: `도보 ${item.transitDurationMin}분 — 버스/택시 권장`,
        dayNumber: item.dayNumber,
        orderIndex: item.orderIndex,
      });
    }

    // 3. 디저트/간식이 식사 slot에 배치
    const startMin = toMinutes(item.startTime);
    const isLunchSlot = startMin >= 660 && startMin <= 870;   // 11:00~14:30
    const isDinnerSlot = startMin >= 1020 && startMin <= 1290; // 17:00~21:30
    if ((isLunchSlot || isDinnerSlot) && (item.category === 'restaurant' || item.category === 'cafe')) {
      const isNonMeal = KNOWN_NON_MEAL.some(n => name.includes(nameLower(n)));
      if (isNonMeal) {
        issues.push({
          type: 'non_meal_in_slot',
          item: item.placeNameSnapshot,
          detail: `디저트/간식이 ${isLunchSlot ? '점심' : '저녁'} slot에 배치`,
          dayNumber: item.dayNumber,
          orderIndex: item.orderIndex,
        });
      }
...
/**
 * 코드 검증 이슈에 대한 자동 보정.
 * - walk_too_long → transitMode를 'bus'로 변경
 * - chain_restaurant / non_meal_in_slot → 아이템에 경고 notes 추가 (LLM 교체는 조건부 audit에서)
 */
export function applyCodeFixes(items: TripItem[], issues: QualityIssue[]): TripItem[] {
  const issueMap = new Map<string, QualityIssue[]>();
  for (const issue of issues) {
    const key = `${issue.dayNumber}-${issue.orderIndex}`;
    if (!issueMap.has(key)) issueMap.set(key, []);
    issueMap.get(key)!.push(issue);
  }

  return items.map(item => {
    const key = `${item.dayNumber}-${item.orderIndex}`;
    const itemIssues = issueMap.get(key);
    if (!itemIssues) return item;
    ...
    for (const issue of itemIssues) {
      switch (issue.type) {
        case 'walk_too_long':
          // 도보 15분 초과 → 버스/택시로 변경
          updated.transitMode = 'bus';
          updated.transitSummary = `버스 ${Math.max(5, Math.round((item.transitDurationMin || 15) / 3))}분 (도보 ${item.transitDurationMin}분 → 대중교통 권장)`;
          updated.transitDurationMin = Math.max(5, Math.round((item.transitDurationMin || 15) / 3));
          break;
        case 'chain_restaurant':
          // 체인 → notes에 경고 추가 (실제 교체는 조건부 LLM audit에서)
          updated.notes = `⚠️ 체인점 — 현지 독립 식당으로 교체 권장. ${item.notes}`;
          break;
        case 'non_meal_in_slot':
          // 디저트가 식사 slot → notes에 경고 추가
          updated.notes = `⚠️ 간식/디저트 — 정식 식당으로 교체 권장. ${item.notes}`;
          break;
      }
    }
    return updated;
