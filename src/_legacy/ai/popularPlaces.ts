...
export interface PopularPlace {
  ...
  photoUrl?: string | null; // Wikipedia 사진 URL (무료 fallback)
}
...
export async function getPopularPlaces(
  ...
  if (hasGoogleKey && supabase) {
    try {
      ...
      if (places.length > 0) {
        ...
        // 사진 없는 장소에 Wikipedia fallback
        try {
          const { getPlacePhoto } = await import('@/lib/services/wikipedia.service');
          for (const place of filtered) {
            if (!place.photoReference) {
              const wiki = await getPlacePhoto(place.name);
              if (wiki) place.photoUrl = wiki.thumbnailUrl;
            }
          }
        } catch { /* 무시 */ }
        return filtered;
  ...
  // 폴백: AI 또는 mock
  let results: PopularPlace[];
  if (providerType === 'mock' || !hasGoogleKey) {
    results = getMockPopularPlaces(destination, excludedPlaces);
  } else {
    results = await getAIPopularPlaces(destination, excludedPlaces, visitedPlaces, userProfile);
  }

  // 사진 없는 장소에 Wikipedia 사진 추가 (무료)
  try {
    const { getPlacePhoto } = await import('@/lib/services/wikipedia.service');
