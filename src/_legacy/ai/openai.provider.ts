...
async function callOpenAI(
  ...
  const response = await getClient().chat.completions.create({
    ...
    ...(isReasoningModel(model)
      ? { max_completion_tokens: maxCompletionTokens }
      : { max_tokens: maxCompletionTokens, temperature: 0.7 }),
    // JSON 모드: 비게이트웨이 + 비reasoning 모델일 때만
    ...(!isGatewayMode() && !isReasoningModel(model)
...
export const openaiProvider: AIProvider = {
  async generateItinerary(
    ...
    if (isGatewayMode()) {
      const compactSystem = buildSystemPrompt('openai', true); // 게이트웨이: compact 모드로 토큰 절약
    ...
    const systemPrompt = buildSystemPrompt('openai', isGatewayMode());
